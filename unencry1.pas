program unencrypt;
  ...
  var lastdir,username,userid,passkey,temp,path:string;
      ...
      name:namestr;
      ext:extstr;
      subdirlen,errorcode,error,filecount:integer;
  ...
  procedure geninvtable(var t:tabletype;var invt:tabletype);
  ...
  procedure checkseg(var x:arraytype;size:integer;var invt:tabletype;var parity:integer);
  ...
  procedure unencriptseg(var x:arraytype;size:integer;var invt:tabletype);
  ...
  procedure unencriptfile(filename:string;passkey:string;var errorcode:integer);

    var x:parraytype;
        t,invt:tabletype;
        numread,numwritten:word;
        earmark,enckey,curpos,filetime,filelength:longint;
        error,signature,filesignat,parity,parkey:integer;
        f:file;
        rndstat:rndinfotype;
        alreadyunenc:boolean;
    ...
    begin
      ...
      alreadyunenc := true;
      if filelength >= 6 then
        begin
          seek(f,filelength-4);
          blockread(f,earmark,4,numread);
          alreadyunenc := (earmark <> SENTINAL);
          seek(f,0)
        end;
      if not alreadyunenc then
        begin
          parity := 0;
          getrndstat(rndstat);
          curpos := 0;
          while (curpos < filelength - 6) do
            begin
              blockread(f,x^,minval(MAXARRAY,filelength - curpos - 6),numread);
              checkseg(x^,numread,invt,parity);
              curpos := curpos + numread
            end;
          blockread(f,filesignat,2,numread);
          signature := parity + parkey;
          setrndstat(rndstat);
          if signature <> filesignat then
            errorcode := 1
          else
            begin
              getftime(f,filetime);
              ...
              while (not eof(f)) do
                begin
                  curpos := filepos(f);
                  blockread(f,x^,MAXARRAY,numread);
                  unencriptseg(x^,numread,invt);
                  seek(f,curpos);
  ...
  begin
    ...
    write('Enter files to unencrypt: ');
        ...
        if p^.data.size > 0 then
          begin
            if fileacc(p^.data.name) then
              begin
                unencriptfile(p^.data.name,passkey,errorcode);
                if errorcode = 0 then
                  begin
                    writeln(fexpand(p^.data.name)+' unencrypted successfully.');
                    filecount := filecount + 1
                  end
                else if errorcode = 2 then
                 writeln(fexpand(p^.data.name)+' is already unencrypted.')
                else if errorcode = 1 then
                  writeln(fexpand(p^.data.name)+':  Data false or encryption key incorrect.')
              end
    ...
    writeln;
    writeln(filecount,' files unencrypted successfully.');
    {$I-}
      chdir(lastdir);
    {$I+}
    cleardirectory(top)
